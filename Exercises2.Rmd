...
Consider the following data. Set the random seed using `set.seed(3000)`. For $n=200$, let $y_{1},\dots y_{n/2}=-1$ and $y_{n/2+1},\dots y_{n}=1$. For each $i$ generate $\boldsymbol{x}_{i}$, if $y_i=-1$, then from a multivariate normal distribution with mean $\boldsymbol{\mu}_1^\top=(0,-1)$ and covariance $\bf{I}$, if $y_i=1$, then from a multivariate normal distribution with mean $\boldsymbol{\mu}_2^\top=(0,1)$ and covariance $\bf{I}$.
...
$$\min_{\boldsymbol{\theta}\in\mathbb{R}^{d+1}}\text{ }\left\{ f\left(\boldsymbol{\theta}\right)=\frac{1}{n}\sum_{i=1}^{n}\left[1-y_{i}\left(\alpha+\boldsymbol{\beta}^{\top}\boldsymbol{x}_{i}\right)\right]_{+}^{2}+\lambda\sum_{j=1}^{d}\left|\beta_{j}\right|\right\}.$$
